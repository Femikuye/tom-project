<?php
	 ...
	 {	 
		...
		public function get_table_rows_by_a_field($table,$field=null,$data=null){
			if(!is_null($field)){
	    ...
	    public function create_unique_slug($data,$table,$field){
	     ...
	     public function get_latest_advert(){
	     	$sql = "SELECT * FROM aids ORDER BY date_created DESC  LIMIT 5";
	     	$query = $this->db->query($sql);
	     	if($query->num_rows() > 0){
	     		return $query->result_array();
	     	}else{
